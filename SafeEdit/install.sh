...
PKG_INSTALLED="$(sed -n 's/^version: *//p' "$HOME/.claude/skills/safe-edit/SKILL.md" 2>/dev/null | head -1)"
if [ -z "$PKG_INSTALLED" ]; then
  echo "safe-edit v$PKG_VERSION — installing..."
elif [ "$PKG_INSTALLED" != "$PKG_VERSION" ]; then
  echo "safe-edit — upgrading v$PKG_INSTALLED → v$PKG_VERSION..."
else
  echo "safe-edit v$PKG_VERSION — reinstalling (same version)..."
fi
